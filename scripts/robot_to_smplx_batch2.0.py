...
def get_retargeter(robot, smplx_folder, gender):
    key = (robot, str(smplx_folder), gender)
    if key not in RETARGETER_CACHE:
        ik_config_path = REVERSE_IK_CONFIG_DICT.get(robot)
        if ik_config_path is None:
            raise ValueError(f"Reverse IK config not registered for robot: {robot}")
        RETARGETER_CACHE[key] = RobotToSMPLXRetargeting(
            robot_type=robot,
            smplx_model_path=smplx_folder,
            gender=gender,
            ik_config_path=ik_config_path,
        )
    return RETARGETER_CACHE[key]
...
def convert_robot_motion(
    ...
    retargeter = get_retargeter(robot, smplx_folder, gender)
    smplx_params = retargeter.motion_to_smplx_params(
        root_pos=root_pos,
